...
public class Main extends ListenerAdapter {
    public static void main(String[] args) {
        ...
        commands.addCommands(
                Commands.slash("ban", "Ban a user from this server. Requires permission to ban users.").addOptions(new OptionData(USER, "user", "The user to ban") // USER type allows to include members of the server or other users by id
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        ...
        switch (event.getName()) {
            case "ban" -> {
                Member member = Objects.requireNonNull(event.getOption("user")).getAsMember(); // the "user" option is required, so it doesn't need a null-check here
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        switch (type) {
            case "prune":
                ...
                event.getChannel().getIterableHistory().skipTo(event.getMessageIdLong()).takeAsync(amount).thenAccept(channel::purgeMessages);
    ...
    public void ban(SlashCommandInteractionEvent event, User user, Member member) {
        ...
        String reason = event.getOption("reason", () -> "Banned by " + event.getUser().getAsTag(), // used if getOption("reason") is null (not provided)
                OptionMapping::getAsString); // used if getOption("reason") is not null (provided)

        // Ban the user and send a success response
        event.getGuild().ban(user, delDays, TimeUnit.DAYS).reason(reason) // audit-log ban reason (sets X-AuditLog-Reason header)
    ...
    public void prune(SlashCommandInteractionEvent event) {
        ...
        event.reply("This will delete " + amount + " messages.\nAre you sure?") // prompt the user with a button menu
                .addActionRow(// this means "<style>(<id>, <label>)", you can encode anything you want in the id (up to 100 characters)
                        Button.secondary(userId + ":delete", "Nevermind!"), Button.danger(userId + ":prune:" + amount, "Yes!")) // the first parameter is the component id we use in onButtonInteraction above
                .queue();
    }
}